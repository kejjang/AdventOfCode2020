...
class Operator(OperatorBase):
    ...
    def __resort_tiles(self):
        tile_pos = [i for i, v in enumerate(self.data) if v[0:4] == "Tile"]
        self.__tiles = {}
        self.__borders = []
        for pos in tile_pos:
            tile_id = int(self.data[pos][5:-1])
            tile_data = []
            for i in range(pos + 1, pos + 11):
                tile_data += [list(self.data[i])]
            tile_border = []
    ...
    def __flip_tile(self, tile_id):
        pos = self.data.index(f"Tile {tile_id}:")
        for i in range(10):
            self.data[pos + i + 1] = ("".join(self.__tiles[tile_id]["data"][i]))[::-1]
        self.__resort_tiles()
